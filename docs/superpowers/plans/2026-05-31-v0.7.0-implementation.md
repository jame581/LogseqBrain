...
1. **Argument:** the user provided a path inline (e.g., "load brain at /path/to/graph"). Use it directly.
2. **Environment variable:** read `LOGSEQ_BRAIN_PATH`. If set and non-empty and the path exists, use it. When the env var wins, don't read or write the config file **for path resolution** — other skills may still read it for non-path settings (e.g. the `journeyLog` toggle, per `journey-log.md`).
...
4. **One-time legacy migration:** if no user config file exists yet, check for a legacy `.brain-config.json` at the plugin root. If found, valid, and its `graphPath` exists, copy all its keys into the user config file (creating the directory if needed), then use its `graphPath`. Silent best-effort — if missing/invalid or the path no longer exists, fall through to step 5.
