...
**Fixture graph.** Copy `fixture-graph/` somewhere outside the repo, add it in Logseq desktop (Add graph), let it index, close Logseq, then run the oracle with `--graph <copy>`. The `Probes` bullet settles which of `. ; : ! ? '` start a tag. Run on 2026-09-12, it showed that `.` and `;` do and `:`, `!`, `?`, `'` do not — and that `*` does too, which the earlier "verified" set had wrong. A `MISSED:` line means `lint.awk`'s exclusion list is wrong for that character: remove it from the ``index(" \t#[]`,\":!?'", nx)`` string, add a `lint-tags` case line, and update `bare-hash-tag` in `skills/_shared/hygiene-rules.md`. A `NOT A LOGSEQ PAGE` line on a real graph means the opposite, so find the occurrence before changing anything: that is how `**C#**` showed that a `*` closing an already-open `**` span is emphasis, not a tag.
