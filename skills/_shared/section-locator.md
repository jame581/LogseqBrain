...
2. **Locate sections** with Grep:
   - `output_mode: "content"`
   - `-n: true`
   - pattern: `^-? ## (SectionName1|SectionName2|...)` — the optional `- ` tolerates both our freshly-written `- ## Heading` form and Logseq's normalized `## Heading` form (see `skills/_shared/logseq-format.md`). List every section you might need in a single Grep call.
