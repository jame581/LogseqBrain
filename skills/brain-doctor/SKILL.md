...
6. **Verify.** Re-run the detections. Confirm zero remaining (excluding intentional forward-references the user chose to keep) and that backtick counts per file are even (no broken inline-code spans). Then, for every **digest-bearing page** (per `skills/_shared/digest.md`'s scope — `pages/Projects___*.md` with `type:: project`, and `pages/Tasks___*.md`; not journals, `Index.md`, `Meta.md`, `Decisions.md`, session-archive pages, or `type:: task-index` / `project-note` auxiliary pages, none of which carry a digest) whose bytes changed during repair, **Remap** its digest per `skills/_shared/digest.md` — recompute the Map bullet only, leaving `digest-updated::` and the prose slots untouched; a repair carries no session knowledge to refresh them from, so this is a Remap, not a Refresh (bumping `digest-updated::` would silence `stale-digest` for another 30 days on prose nobody touched). A repair is exactly the kind of byte-moving write that leaves a stale Map behind (see `stale-map`) if the digest isn't recomputed alongside it — but a repair to a journal, `Index.md`, `Meta.md`, or `Decisions.md` has no Map to leave stale in the first place.
