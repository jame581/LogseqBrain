...
4. **Read today's journal's mention of this project, bounded** (`journals/yyyy_MM_dd.md`) if the file exists — not the whole journal, which aggregates every project worked on that day and can run past 10 KB (measured on the reference graph: 10,152 B). Grep the journal's `## Sessions` section for the bullet naming `[[Projects/<MatchedName>]]` and read just that bullet (and its children, if any) — bounded per the journal budget row in `skills/_shared/section-locator.md`, ≤ ~2 KB. No match → nothing to read for this project today, move on. This captures session notes from earlier today without pulling in every other project's activity.
...
3. Plus today's journal's mention of this project, if any (step 4 above) — bounded to ≤ ~2 KB, not the whole journal file. Nothing else beyond the digest and that one bounded journal check. ≤ ~4 KB total regardless of page size (≤ ~2 KB digest + ≤ ~2 KB journal mention) — not the "≤ ~2 KB, nothing else" this step claimed before wave E3, which silently ignored the journal read every digest-mode load already performs.
