...
Check the page total and the Session Log entry count `brain-save` step 3 already recorded from `brain sections <page>` — its baseline call, made before this session's appends. Don't call `sections` again to get a post-write figure: that resets the baselines `brain check` diffs against (`brain-save`'s Important Notes). The check is against:
