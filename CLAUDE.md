...
A Claude Code plugin (`logseq-brain`) that gives Claude persistent memory via a user-owned Logseq graph. The plugin is markdown skills (`skills/<name>/SKILL.md`), `.claude-plugin/plugin.json`, and, since v0.11.0, **one POSIX `sh` + `awk` helper**: `skills/_shared/bin/brain` plus `skills/_shared/lib/*.awk`. Claude does the judgment (what to save, the prose). The helper does every mechanical step: section measurement, the digest Map, caps, scoped search, lint, the activity line. This deliberately reverses the earlier "no runtime code" rule, within a narrow boundary: POSIX sh/awk only, no new dependencies, and the script writes exactly four kinds of line: the Map line, one activity bullet, one `## Sessions` bullet, and the parenthetical of a project's `pages/Index.md` one-liner (`docs/superpowers/specs/2026-09-11-v0.11.0-design.md`; the last two since v0.12.0, `docs/superpowers/specs/2026-09-25-v0.12.0-design.md` D1).

**Target: Logseq OG (the markdown version) only.** Logseq split in 2026 — OG moved to <https://github.com/logseq/og> and is in maintenance mode (security and Electron upgrades, no new features), while the DB/SQLite version continues at the original repo. There is no API or CLI for file graphs (`@logseq/cli` serves DB graphs only), so all leverage here is file layout, property discipline, and the helper's own awk search. Do not propose DB-version features.
...
Cross-skill logic lives under `skills/_shared/` — sibling to the skill folders, not inside any individual skill's `references/`. Each `SKILL.md` reads from `skills/_shared/<name>.md` on demand. This keeps `SKILL.md` orchestrators compact and avoids duplicating logic across skills. One of these is still large — `hygiene-rules.md` (18 KB) — so grep to the relevant rule rather than reading it whole. Developer-only contract text (the Map computation, lint detection detail) lives in `docs/reference/`, which is not shipped.
...
`skills/_shared/bin/brain` is a sh dispatcher. Each command is an awk program loaded after `lib/core.awk` (`awk -f core.awk -f <prog>.awk`). Every `SKILL.md` carries the same invocation block: via `sh`, or via Git's `bash.exe` on PowerShell-only Windows hosts. `skills/_shared/run-brain.md` is the full command reference. Since v0.12.0 a routine load is one `brain load` call, and a save is `brain save-begin`, the model's Edits, then `brain save-finish` (composites: shell functions over the existing commands). It is required, and there is no prose fallback. Portability rules are non-negotiable: `LC_ALL=C` (lengths are bytes), POSIX awk only (no `gensub`, three-argument `match`, `strftime`/`mktime`, `{n,m}` intervals, or `length(array)`), and graph-relative paths in output (`brain info`'s `graph:` line is absolute by design). Its only writes are the Map line (`digest --apply`, `save-finish`), one activity bullet (`activity`, `load`, `save-finish`), and, from `save-finish`, one `## Sessions` bullet and the Index parenthetical. Every write is verified to touch only its line(s) before an in-place commit, and Claude-composed text is linted before it is written.
...
The compose-time rules (backticks, `#`-escaping, namespaced links, file links) are documented in full in `skills/_shared/logseq-format.md`, checked on every save by `save-finish`, and whole-graph by `brain-doctor`.
...
- **One branch per minor version.** Work happens on a `vX.Y.Z` branch merged to `main` by PR (#1–#5 did; the eval suite merged directly); commits are conventional with a scope — `feat(brain-init):`, `fix(digest):`, `docs(claude-md):`.
