...
- **brain-doctor** — Graph-hygiene lint/repair (v0.8.0). Scans pages + journals for format violations that spawn phantom pages or broken macros (`{{ }}` mis-used for inline code, bare `#number`/hex tags, un-namespaced `[[Task]]` links, `[[file://]]` links, junk/description links), reports them, and — after a backup — repairs them. Includes the `jira-markup` residue check and the guided task-status backfill. Maintenance tool, run on demand; not part of the per-session save/load cycle.
